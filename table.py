...
def create_score_table(table: List[Dict[str, any]]):
    ...
    score_table = dash_table.DataTable(
        ...
        data=table,
        columns=columns,#[
            #{"name": i, "id": i, "renamable": True, "deletable": True}
            #for i in list(table[0].keys())
        #],
        editable=True,
    )
    return score_table
...
def layout(date: str):
    date_verbose = datetime.datetime.strptime(date, "%m%Y").strftime("%B %Y") 
    top_header = html.H1(children=f"Bjorli games {date_verbose}", style={"text-align": "center"})
    newest_file = sorted(Path(f"data/{date}").iterdir())[-1]
